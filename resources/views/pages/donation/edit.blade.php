...
<a class="bg-orange-300 hover:bg-orange-500 shadow-md px-5 py-2.5 rounded-md text-white" href="{{ route('donation.index') }}">< Kembali</a>
    ...
    <form method="POST" action="{{ route('donation.update',$donation->id) }}" class="max-w-md mx-auto shadow-md px-10  py-6 rounded-b-md">
        ...
        <div class="grid grid-cols-2 gap-6">
            <div class="relative z-0 w-full mb-5 group">
                <input type="number" name="add" id="add" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "/>
                <label for="add" class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Tambah</label>
            </div>
            <div class="relative z-0 w-full mb-5 group">
                <input type="number" name="diff" id="diff" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "/>
